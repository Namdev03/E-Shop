...
export function ResetPasswordEmail({ role = "user" }) {
  ...
  const onSubmit = async ({ password }) => {
    try {
      const { data } = await axiosInstance.post(`${base}/reset-password/email/${token}`, { password });
  ...
  return (
    <div className="flex min-h-[calc(100vh-64px)] items-center justify-center bg-[#FAFAFA] px-4 py-12">
      <div className="w-full max-w-md rounded-2xl border border-[#E5E7EB] bg-white p-8 shadow-sm">
        <h1 className="mb-6 text-center text-2xl font-bold text-[#101828]">Reset your password</h1>
...
function Field({ icon: Icon, label, type, error, inputProps }) {
  return (
    <div>
