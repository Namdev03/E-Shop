...
import { ShoppingBag, User, Mail, Phone, Lock } from "lucide-react";
    ...
    <div className="flex min-h-[calc(100vh-64px)] items-center justify-center bg-[#FAFAFA] px-4 py-12">
      <div className="w-full max-w-md rounded-2xl border border-[#E5E7EB] bg-white p-8 shadow-sm">
        <div className="mb-6 text-center">
          ...
          <h1 className="text-2xl font-bold text-[#101828]">Create your account</h1>
          <p className="mt-1 text-sm text-gray-500">Join E-Shop and start saving</p>
        </div>

        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <Field icon={User} label="Full Name" error={errors.fullName} inputProps={register("fullName", { required: "Full name is required" })} placeholder="Jane Doe" />
        ...
        <p className="mt-6 text-center text-sm text-gray-500">
          ...
          <Link to="/login" className="font-semibold text-[#C81E5C] hover:underline">Log in</Link>
        </p>
      </div>
    </div>
  );
