...
import { loginSeller } from "../../redux/slices/sellerSlice.js";
...
export function SellerLogin() {
  ...
  const onSubmit = async (formData) => {
    const result = await dispatch(loginSeller({ ...formData, rememberMe }));
    ...
    <div className="flex min-h-[calc(100vh-64px)] items-center justify-center bg-[#FAFAFA] px-4 py-12">
      <div className="w-full max-w-md rounded-2xl border border-[#E5E7EB] bg-white p-8 shadow-sm">
        <div className="mb-6 text-center">
          ...
          <h1 className="text-2xl font-bold text-[#101828]">Seller Login</h1>
          <p className="mt-1 text-sm text-gray-500">Manage your store and orders</p>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="flex items-center justify-between">
            ...
            <Link to="/seller/forgot-password" className="text-xs font-medium text-[#101828] hover:underline">
              Forgot password?
            </Link>
          </div>
        ...
        <p className="mt-6 text-center text-sm text-gray-500">
          New seller?{" "}
          <Link to="/seller/signup" className="font-semibold text-[#101828] hover:underline">Create a store</Link>
        </p>
      </div>
    </div>
  );
